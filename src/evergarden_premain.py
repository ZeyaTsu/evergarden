...
def on_download_clicked():
    ...
    try:
        ...
        c = c["evergarden"]
        anime_name = c["anime_name"]
        character_name = c["character_name"]
        if c["gif"] == 'True':
            gif_only = True
        else:
            gif_only = False
        num_images_to_download = int(c["n_images"])
        ...
        image_type = c["type"]
        logs(f"Set loaded and used : {set_loaded}")
...
if set_loaded:
    label_anime_name = tk.Label(main_tab, text="Set loaded", fg="white", bg="#0d1b2a")
    label_anime_name.grid(row=6, column=1, columnspan=3,padx=5, pady=10)
